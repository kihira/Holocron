...
export function format(input: string): string {
    return input.replace(/\[CHECK:([A-Z]+):?([a-zA-Z]*)\]/g, (match, p1, p2) => `**${check.get(p1)} ${idToName(p2)} check**`)
                .replace(/\[CHARACTERISTIC:([a-zA-Z]+)\]/g, "**$1**")
                .replace(/\[([A-Z]+):?([a-zA-Z]*)\]/g, (match, p1, p2) => `**${check.get(p1)} ${idToName(p2)}**`)
                .replace(/\[([A-Z]+)\]/g, (match) => emojiMap.get(match) || "");
}
